...
public class AiInvestService {
    ...
    public AiInvestService(OpenAiService openAiService, KiwoomKrStockService kiwoomKrStockService) {
        this.openAiService = openAiService;
        this.kiwoomKrStockService = kiwoomKrStockService;
    }

    // -------------------------------------------------------------------------
    // Public helpers used by AiInvestThreadService
    // -------------------------------------------------------------------------

    public String normalizeStockName(String raw) {
        if (raw == null || raw.isBlank()) {
            throw new IllegalArgumentException("종목명 또는 종목코드를 입력하세요.");
        }
        String normalized = raw.trim();
        return normalized.length() > 60 ? normalized.substring(0, 60) : normalized;
    }

    public String normalizeMarket(String raw) {
        if (raw == null || raw.isBlank()) {
            return "국내";
        }
        String normalized = raw.trim();
        if (!"국내".equals(normalized) && !"해외".equals(normalized)) {
            throw new IllegalArgumentException("시장 구분은 \"국내\" 또는 \"해외\" 중 하나여야 합니다.");
        }
        return normalized;
    }
    ...
    public String buildThreadTitle(String stockName, String market, String tradeStyle) {
        String title = stockName + " · " + market + " · " + tradeStyle + " 전략";
    ...
    public String buildInitialPrompt(
        ...
        return """
                ...
                """.formatted(stockName, market, riskProfile, tradeStyle, memoLine).trim();
    }
    ...
    public OpenAiService.WebSearchResult adviseConversation(
        ...
        OpenAiService.WebSearchResult result = openAiService.respondWithWebSearch(
                buildSystemPrompt(),
                buildConversation(stockName, market, riskProfile, tradeStyle, notes, conversation)
        );
    ...
    private List<OpenAiService.ChatTurn> buildConversation(
        ...
        turns.add(new OpenAiService.ChatTurn("user",
                buildSessionContext(stockName, market, riskProfile, tradeStyle, notes)));
    ...
    private String buildSessionContext(
        ...
        String chartContext = "국내".equals(market) ? buildKrChartContext(stockName, tradeStyle) : "";
        return """
                ...
                %s
                """.formatted(stockName, market, riskProfile, tradeStyle, noteLine, chartContext).trim();
    }

    /**
     * For domestic stocks where stockName looks like a 6-digit code,
     * attempt to fetch chart context from DB금융투자 API.
     * All failures are silently swallowed.
     */
    private String buildKrChartContext(String stockName, String tradeStyle) {
        String code = extractKrCode(stockName);
        if (code == null) {
            return "";
        }

        try {
            KrStockSnapshotResponse snapshot = kiwoomKrStockService.getSnapshot(code);
            List<KrStockCandleResponse> dailyCandles = snapshot.getDailyCandles() == null
                    ? List.of() : snapshot.getDailyCandles();
            List<KrStockCandleResponse> intradayCandles = snapshot.getIntradayCandles() == null
                    ? List.of() : snapshot.getIntradayCandles();
            List<KrStockCandleResponse> selectedCandles =
                    selectCandlesForTradeStyle(tradeStyle, intradayCandles, dailyCandles);
            ...
            List<KrStockCandleResponse> anchorWindow = lastItems(
                    selectedCandles, Math.min(selectedCandles.size(), "스윙".equals(tradeStyle) ? 12 : 18));
            double support = average(
                    anchorWindow.stream().map(KrStockCandleResponse::getLow).sorted().limit(3).toList());
            double resistance = average(
                    anchorWindow.stream()
            ...
            double currentPrice = (snapshot.getQuote() != null && snapshot.getQuote().getLastPrice() != null)
            ...
            log.debug("키움 chart context fetch failed silently for {}: {}", stockName, ex.getMessage());
            return "";
        }
    }

    /**
     * Returns a bare 6-digit code if stockName looks like one; otherwise null.
     */
    private String extractKrCode(String stockName) {
        if (stockName == null) {
            return null;
        }
    ...
    // -------------------------------------------------------------------------
    // Candle helpers (same logic as StockAdvisorService)
    // -------------------------------------------------------------------------
    ...
    private List<KrStockCandleResponse> aggregateIntradayCandles(
            List<KrStockCandleResponse> candles, int minuteBucket) {
    ...
    private String buildSystemPrompt() {
        return """
                ...
                - If the stock name or code is not a common operating company stock, adapt the analysis and explain the limits.
                - Do not promise profit, certainty, or guaranteed upside.
                - Treat the conversation as one continuous advisory session for the same stock unless the user explicitly changes it.
